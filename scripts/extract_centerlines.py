...
4. 起始部(内腔半径が最大の端点。オスティウムはその血管で最も太く、末梢へ向かって
   単調に先細るため)を根とし、各分岐点で「進入方向との角度(直進性)を
...
def process_vessel(scene: trimesh.Scene, vessel_id: str) -> dict:
    ...
    # 起始部(オスティウム)は解剖学的にその血管で最も内腔径が太い点(そこから末梢へ
    # 向かって単調に先細っていく)なので、末端ノードの中で半径最大のものを根に選ぶ。
    # 以前は「Y座標が最大(=最も頭側)」で選んでいたが、RCAでは近位で分岐する細い
    # 枝(円錐枝/洞結節枝相当)の枝先が本来のオスティウムよりわずかに頭側に位置しており、
    # この細い枝先が誤って根に選ばれていた(実測: 誤った根の半径は0.0104で、本来の
    # オスティウムの半径0.0533の1/5程度しかなく、末端の中でも最小クラス)。半径基準なら
    # LAD/LCXの根(いずれも全末端中で半径最大、2番目に大きい末端の3倍以上)は変わらず、
    # RCAだけ正しいオスティウムに修正される。
    root = max(endpoints, key=lambda n: radii_world[n])
