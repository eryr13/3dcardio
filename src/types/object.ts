 ...
 * (stenosisPlaqueMesh.ts)・石灰化シェル形状(calcificationMesh.ts)が共通でこの
 * 関数を使うことで、「内腔がどれだけ狭くなっているか」の値がどこから参照しても
 * 常に一致する(以前は狭窄プラークの見た目だけガウス関数でテーパーし、造影剤
 * フローが参照する内腔比率は矩形窓のままだったため、プラークの見た目と造影剤
 * チューブの半径が一致せず、区間境界に円錐状の段差が見えていた——石灰化も同様に、
 * 見た目のジオメトリだけ長さ方向に一様な厚みのままで、区間境界にドーナツ状の
 * リング断面が露出していた)。
 */
